...
export default function AnnuaireClient({ partenaires, paysList, userRole }: Props) {
  ...
  const afficherContact = (p: Partenaire) => {
    if (userRole === 'admin') return true
    return p.contact_visible
  }
      ...
      <div style={{ width: 220, minWidth: 220, background: '#fff', borderRight: '1px solid #EEF0F3', padding: '16px 14px', overflowY: 'auto' }}>
        <div style={{ position: 'relative', marginBottom: 16 }}>
          <input
            ...
            style={{ width: '100%', padding: '7px 8px 7px 26px', borderRadius: 8, border: '1.5px solid #E2E8F0', fontSize: 12, boxSizing: 'border-box', outline: 'none', color: '#1A202C' }}
          />
        </div>
        ...
        <select value={filterPays} onChange={e => setFilterPays(e.target.value)} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '1.5px solid #E2E8F0', fontSize: 12, background: '#F8FAFC', outline: 'none' }}>
          <option>Tous</option>
          {paysList.map(p => <option key={p}>{p}</option>)}
        </select>
        <div style={{ marginTop: 12, fontSize: 11, color: '#94A3B8' }}>
          <span style={{ fontWeight: 700, color: '#0A3D26' }}>{filtered.length}</span> resultat(s)
        </div>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr' : '1fr 1fr', gap: 12 }}>
            {filtered.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => { setSelected(p); setEditMode(false) }} style={{ background: '#fff', borderRadius: 12, border: `2px solid ${selected?.id === p.id ? '#0A3D26' : '#EEF0F3'}`, padding: '14px 16px', cursor: 'pointer', transition: 'all 0.15s' }}>
          ...
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {userRole === 'admin' && !editMode && (
                <button onClick={() => ouvrirEdition(selected)} style={{ padding: '4px 10px', borderRadius: 7, border: '1.5px solid #EEF0F3', background: '#F8FAFC', fontSize: 11, cursor: 'pointer', color: '#475569' }}>
                  Modifier
                </button>
              )}
            ...
            {!editMode ? (
              <>
                  ...
                  {selected.telephone && (
                    <div style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: '#1A202C' }}>
                        {selected.telephone_indicatif} {selected.telephone}
                      </span>
                    </div>
                  )}
                  {selected.site_web && (
                ...
                {afficherContact(selected) && (selected.contact_nom || selected.contact_prenom) && (
                  <div style={{ marginBottom: 14, padding: '12px 14px', borderRadius: 10, background: '#F0FDF4', border: '1px solid #A7F3D0' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#065F46', marginBottom: 6, textTransform: 'uppercase' }}>
                      Contact {!selected.contact_visible && userRole === 'admin' && <span style={{ fontSize: 9, background: '#FEF3C7', color: '#92400E', padding: '1px 6px', borderRadius: 10, marginLeft: 4 }}>Admin seulement</span>}
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#0A3D26' }}>
                      {selected.contact_prenom} {selected.contact_nom}
                    </div>
                ...
                {selected.description && (
                  <p style={{ fontSize: 12, color: '#64748B', lineHeight: 1.6, marginBottom: 14 }}>{selected.description}</p>
                )}
              </>
            ) : (
              /* Mode edition */
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <select value={form.telephone_indicatif ?? '+33'} onChange={e => setForm(f => ({ ...f, telephone_indicatif: e.target.value }))} style={{ ...inputStyle, width: 90, flexShrink: 0 }}>
                ...
                <div style={{ padding: '12px', borderRadius: 10, background: '#F8FAFC', border: '1px solid #EEF0F3' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#0A3D26', marginBottom: 10, textTransform: 'uppercase' }}>Contact</div>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                    <input
                      type="checkbox"
                      checked={form.contact_visible ?? false}
                      onChange={e => setForm(f => ({ ...f, contact_visible: e.target.checked }))}
                      style={{ accentColor: '#0A3D26', width: 14, height: 14 }}
                    />
                    <span style={{ fontSize: 12, color: '#475569' }}>Rendre ce contact visible par tous les utilisateurs</span>
                  </label>
                  <div style={{ fontSize: 10, color: '#94A3B8', marginTop: 4 }}>
                    Si non coche, le contact reste visible uniquement par les administrateurs.
                  </div>
                </div>
