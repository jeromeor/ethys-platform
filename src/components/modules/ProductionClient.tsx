...
export default function ProductionClient({ commandes: initial, user, role }: Props) {
  ...
  const chargerCertifications = async () => {
   const { data } = await supabase.from('certifications_ethys').select('id, reference, type_produit, statut').eq('statut', 'certifiee').is('lot_id', null).order('created_at', { ascending: false })
