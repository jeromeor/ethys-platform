...
export default function AdminEntreprisesClient({ entreprises: initial }: Props) {
  const t = useTranslations('adminEntreprises')
  ...
  // Libellés traduits dérivés des IDs stables
  ...
  const statutLabel = (statut: string) => {
    const map: Record<string, string> = {
      actif: t('statutActif'),
      inactif: t('statutInactif'),
      verifie: t('statutVerifie'),
      en_cours: t('statutEnCours'),
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, padding: '12px 22px', flexShrink: 0 }}>
        {[
          { label: t('kpiTotal'), value: '' + entreprises.length },
          { label: t('kpiMarques'), value: '' + entreprises.filter(e => e.type === 'marque').length },
          { label: t('kpiFilatures'), value: '' + entreprises.filter(e => e.type === 'filature').length },
          { label: t('kpiFournisseurs'), value: '' + entreprises.filter(e => e.type === 'fournisseur_coton').length },
        ].map((k, i) => (
      ...
      <div style={{ padding: '10px 22px', background: '#fff', borderBottom: '1px solid #e8e3d8', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {['tous', 'marque', 'filature', 'fournisseur_coton'].map(tt => (
            <button key={tt} onClick={() => setFilterType(tt)} style={{
              padding: '5px 12px', borderRadius: 4, border: 'none', cursor: 'pointer',
              fontSize: 11, fontWeight: filterType === tt ? 700 : 500,
              background: filterType === tt ? '#1a1a1a' : '#f5f3ef',
              color: filterType === tt ? '#fff' : '#4a5568'
            }}>
              {tt === 'tous' ? t('filtreToutes') : typeLabel(tt)}
            </button>
          ))}
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px 22px' }}>
        {filtrees.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px', color: '#8b7355', fontSize: 13 }}>{t('aucuneEntreprise')}</div>
        ) : (
          <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #e8e3d8', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f5f3ef' }}>
                  {[t('hNom'), t('hType'), t('hVille'), t('hPays'), t('hEmail'), t('hStatut'), t('hActions')].map(h => (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 700, background: tbg, color: ttc }}>{typeLabel(e.type)}</span>
                      </td>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 700, background: sbg, color: stc }}>{statutLabel(e.statut)}</span>
                      </td>
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', gap: 6 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }} onClick={() => setShowForm(false)}>
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '90vh', overflow: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 26px', borderBottom: '1px solid #f5f3ef', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: '#1a1a1a' }}>{editTarget ? t('modalTitreModifier', { nom: editTarget.nom }) : t('modalTitreCreer')}</span>
            ...
            <div style={{ padding: '20px 26px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={labelStyle}>{t('labelNom')}</label>
                  <input style={inputStyle} value={form.nom} onChange={e => set('nom', e.target.value)} placeholder={t('placeholderNom')} />
                </div>
                <div>
                  <label style={labelStyle}>{t('labelType')}</label>
                  <select style={inputStyle} value={form.type} onChange={e => set('type', e.target.value)}>
                    <option value="marque">{t('typeMarque')}</option>
                    <option value="filature">{t('typeFilature')}</option>
                    <option value="fournisseur_coton">{t('typeFournisseur')}</option>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={labelStyle}>{t('labelSiret')}</label>
                  <input style={inputStyle} value={form.siret} onChange={e => set('siret', e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t('labelTva')}</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 2fr', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={labelStyle}>{t('labelCodePostal')}</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div>
                  <label style={labelStyle}>{t('labelEmailContact')}</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                <div>
                  <label style={labelStyle}>{t('labelSiteWeb')}</label>
                  <input style={inputStyle} value={form.site_web} onChange={e => set('site_web', e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t('labelStatut')}</label>
                  <select style={inputStyle} value={form.statut} onChange={e => set('statut', e.target.value)}>
                    <option value="actif">{t('statutActif')}</option>
                    <option value="inactif">{t('statutInactif')}</option>
                    <option value="verifie">{t('statutVerifie')}</option>
                    <option value="en_cours">{t('statutEnCours')}</option>
                  </select>
                </div>
              </div>

              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setShowForm(false)} style={{ flex: 1, padding: '10px', borderRadius: 4, border: '1.5px solid #e8e3d8', background: '#f5f3ef', color: '#8b7355', fontSize: 13, cursor: 'pointer' }}>{t('btnAnnuler')}</button>
                <button onClick={sauvegarder} disabled={saving} style={{ flex: 2, padding: '10px', borderRadius: 4, border: 'none', background: saving ? '#d4c5b0' : '#1a1a1a', color: saving ? '#8b7355' : '#fff', fontSize: 13, fontWeight: 700, cursor: saving ? 'default' : 'pointer' }}>
                  {saving ? t('btnEnregistrer') : (editTarget ? t('btnEnregistrerMaj') : t('btnCreer'))}
                </button>
              </div>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }} onClick={() => setDeleteTarget(null)}>
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 460, boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 26px', borderBottom: '1px solid #fde8e8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: '#8b3a3a' }}>{t('deleteTitre')}</span>
            ...
            <div style={{ padding: '20px 26px' }}>
              <div style={{ padding: '12px 16px', borderRadius: 8, background: '#fdf0f0', border: '1px solid #c8a0a0', fontSize: 13, color: '#8b3a3a', fontWeight: 600, marginBottom: 20 }}>
                {t('deleteWarning', { nom: deleteTarget.nom })}
              </div>
              ...
              <div onClick={() => setDeleteConfirm1(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 8, border: '1.5px solid ' + (deleteConfirm1 ? '#8b3a3a' : '#e8e3d8'), background: deleteConfirm1 ? '#fdf0f0' : '#f5f3ef', cursor: 'pointer', marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 12, color: '#4a5568' }}>{t('deleteConfirm1')}</span>
              </div>
              ...
              <div onClick={() => setDeleteConfirm2(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 8, border: '1.5px solid ' + (deleteConfirm2 ? '#8b3a3a' : '#e8e3d8'), background: deleteConfirm2 ? '#fdf0f0' : '#f5f3ef', cursor: 'pointer', marginBottom: 20 }}>
                ...
                <span style={{ fontSize: 12, color: '#4a5568' }}>{t('deleteConfirm2')}</span>
              </div>

              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setDeleteTarget(null)} style={{ flex: 1, padding: '10px', borderRadius: 4, border: '1.5px solid #e8e3d8', background: '#f5f3ef', color: '#8b7355', fontSize: 13, cursor: 'pointer' }}>{t('btnAnnuler')}</button>
