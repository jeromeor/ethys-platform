...
import DemandeCertificationForm from './DemandeCertificationForm'
...
export default function CertificationClient({
  ...
  // Ouvre le formulaire si commande_id en query param (pré-remplissage géré par le composant du formulaire)
  useEffect(() => {
  ...
  const renderPanneauDroit = () => {
    ...
    if (showForm) {
      return (
        <DemandeCertificationForm
          onClose={() => setShowForm(false)}
          commandesDispos={commandesDispos}
          setCommandesDispos={setCommandesDispos}
          setDeclsFilature={setDeclsFilature}
          entrepriseId={entrepriseId}
          userId={userId}
          initialCommandeId={searchParams.get('commande_id') ?? undefined}
        />
      )
    }
