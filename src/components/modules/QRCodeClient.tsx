...
import TransfertQrModal from './TransfertQrModal'
import ApercuPublicModal from './ApercuPublicModal'
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
     ...
     {/* Modal apercu public */}
      {previewPublic && qrActif && selected && (
        <ApercuPublicModal onClose={() => setPreviewPublic(false)} selected={selected} qrActif={qrActif} qrDataUrl={qrDataUrl} />
      )}
      ...
      {selectedCert && source === 'certs' && (
        <div style={{ flex: 1, overflow: 'auto', padding: '24px 28px' }}>
          ...
          {showTransfert && selectedCert && (
        <TransfertQrModal onClose={() => setShowTransfert(false)} marques={marques} selectedCert={selectedCert} profil={profil} />
      )}  
        </div>
      )}
    </div>
