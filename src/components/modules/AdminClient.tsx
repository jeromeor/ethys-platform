  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, padding: '14px 22px', flexShrink: 0 }}>
        {[
          { label: 'Utilisateurs actifs', value: `${nbActifs}`, icon: '◎', bg: '#D1FAE5', tc: '#065F46' },
          { label: 'Alertes sécurité', value: `${nbAlertes}`, icon: '⚠', bg: nbAlertes > 0 ? '#FEE2E2' : '#F1F5F9', tc: nbAlertes > 0 ? '#991B1B' : '#475569' },
          { label: 'Conformité RGPD', value: '100%', icon: '✓', bg: '#F0FDF4', tc: '#065F46' },
          { label: 'Score sécurité', value: `${secScore}%`, icon: '🔒', bg: '#DBEAFE', tc: '#1E40AF' },
        ].map((k, i) => (
          <div key={i} style={{ background: '#fff', borderRadius: 12, border: '1px solid #EEF0F3', padding: '14px 18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: k.bg, color: k.tc, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13 }}>{k.icon}</div>
              <span style={{ fontSize: 11, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 0.3 }}>{k.label}</span>
            </div>
      ...
      <div style={{ display: 'flex', borderBottom: '2px solid #EEF0F3', padding: '0 22px', flexShrink: 0, background: '#fff' }}>
        ...
        <button onClick={() => setInviteModal(true)} style={{
          margin: '8px 0', padding: '6px 14px', borderRadius: 8, border: 'none',
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px 22px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {['Utilisateur', 'Rôle', 'Entreprise', 'Statut', 'Dernière connexion', 'Actions'].map(h => (
                    ...
                    >
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: 9, background: rbg, color: rtc, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0 }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{
                          ...
                          background: u.statut === 'actif' ? '#D1FAE5' : '#F1F5F9',
                          color: u.statut === 'actif' ? '#065F46' : '#94A3B8',
                          display: 'inline-flex', alignItems: 'center', gap: 4
                        }}>
                          <span style={{ width: 5, height: 5, borderRadius: '50%', background: u.statut === 'actif' ? '#10B981' : '#CBD5E1' }} />
                          {u.statut}
                        </span>
                      </td>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          ...
                          {u.id !== currentUserId && (
                            <button onClick={() => toggleStatut(u.id)} style={{
                              padding: '4px 10px', borderRadius: 7, border: 'none',
                              background: u.statut === 'actif' ? '#FEE2E2' : '#D1FAE5',
                              color: u.statut === 'actif' ? '#DC2626' : '#065F46',
                              fontSize: 11, cursor: 'pointer', fontWeight: 600
                            }}>{u.statut === 'actif' ? 'Désactiver' : 'Activer'}</button>
                          )}
                        </div>
                      </td>
                    </tr>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {ROLES_DEF.map((r, i) => (
              <div key={i} style={{ background: '#fff', borderRadius: 14, border: '1px solid #EEF0F3', padding: '18px 22px' }}>
                ...
                <p style={{ fontSize: 12, color: '#64748B', marginBottom: 12, lineHeight: 1.5 }}>{r.desc}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {r.permissions.map((p, j) => (
                    <span key={j} style={{ padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 600, background: r.bg, color: r.couleur }}>
                      ✓ {p}
                    </span>
                  ))}
                </div>
        ...
        {activeTab === 'Journal d\'audit' && (
          <div>
            {nbAlertes > 0 && (
              <div style={{ padding: '12px 16px', borderRadius: 10, marginBottom: 14, background: '#FEF2F2', border: '1px solid #FCA5A5', display: 'flex', gap: 10, alignItems: 'center' }}>
                <span style={{ fontSize: 16 }}>⚠</span>
                <span style={{ fontSize: 12, color: '#991B1B', fontWeight: 600 }}>
                  {nbAlertes} alerte(s) de sécurité détectée(s) dans le journal.
                </span>
              </div>
            )}
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #EEF0F3', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#F8FAFC' }}>
                    {['Niveau', 'Utilisateur', 'Action', 'Cible', 'IP', 'Date'].map(h => (
                ...
                <tbody>
                  {audit.length === 0 ? (
                    <tr><td colSpan={6} style={{ padding: '30px', textAlign: 'center', color: '#94A3B8', fontSize: 12 }}>Aucune entrée dans le journal</td></tr>
                  ) : audit.map((ev, i) => {
                    ...
                    return (
                      ...
                      >
                        <td style={{ padding: '11px 14px' }}>
                          <span style={{ width: 26, height: 26, borderRadius: 7, background: ns.bg, color: ns.tc, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>{ns.icon}</span>
                        </td>
                        <td style={{ padding: '11px 14px', fontSize: 11, fontWeight: 600 }}>{ev.user_email ?? 'Système'}</td>
                        <td style={{ padding: '11px 14px', fontSize: 11, color: '#475569' }}>{ev.action}</td>
                        <td style={{ padding: '11px 14px', fontSize: 11, color: '#64748B', fontFamily: 'monospace' }}>{ev.cible ?? '—'}</td>
                        <td style={{ padding: '11px 14px', fontSize: 11, fontFamily: 'monospace', color: '#94A3B8' }}>{ev.ip_address ?? '—'}</td>
                        <td style={{ padding: '11px 14px', fontSize: 10, color: '#94A3B8' }}>
                          {new Date(ev.created_at).toLocaleDateString('fr-FR')} {new Date(ev.created_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                        </td>
                      </tr>
                    )
                  })}
        ...
        {activeTab === 'Sécurité' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #EEF0F3', padding: '20px 22px' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0A3D26', marginBottom: 14 }}>Paramètres de sécurité</div>
              {params.map((p, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 0', borderBottom: i < params.length - 1 ? '1px solid #F8FAFC' : 'none' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#1A202C' }}>{p.label}</div>
                    <div style={{ fontSize: 11, color: '#94A3B8' }}>{p.desc}</div>
                  </div>
                  <div onClick={() => setParams(prev => prev.map((x, j) => j === i ? { ...x, actif: !x.actif } : x))} style={{
                    width: 38, height: 20, borderRadius: 10, cursor: 'pointer',
                    background: p.actif ? '#0A3D26' : '#CBD5E1',
              ...
              <div style={{ marginTop: 14, padding: '10px 12px', borderRadius: 8, background: '#F8FAFC' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 5 }}>
                  <span style={{ fontWeight: 600, color: '#0A3D26' }}>Score sécurité</span>
                  <span style={{ fontWeight: 800 }}>{secScore}%</span>
                </div>
                <div style={{ height: 5, background: '#E2E8F0', borderRadius: 3 }}>
                  <div style={{ height: '100%', width: `${secScore}%`, background: 'linear-gradient(90deg,#10B981,#0A3D26)', borderRadius: 3, transition: 'width 0.3s' }} />
                </div>
              </div>
            </div>

            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #EEF0F3', padding: '20px 22px' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0A3D26', marginBottom: 14 }}>Conformité RGPD</div>
              {[
                { label: 'Politique de confidentialité', statut: 'À jour', ok: true },
                { label: 'Registre des traitements', statut: 'À jour', ok: true },
                { label: 'Consentements utilisateurs', statut: '100% collectés', ok: true },
                { label: 'DPO désigné', statut: 'Oui', ok: true },
                { label: 'Chiffrement données', statut: 'AES-256 actif', ok: true },
                { label: 'Durée conservation', statut: '3 ans (paramétré)', ok: true },
                { label: 'Droit à l\'oubli', statut: 'Documenté', ok: true },
                { label: 'Transferts hors UE', statut: 'Clauses contractuelles', ok: true },
              ].map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', borderRadius: 8, background: '#F0FDF4', border: '1px solid #A7F3D0', marginBottom: 6 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#1A202C' }}>{item.label}</span>
                  </div>
                  <span style={{ fontSize: 11, color: '#065F46' }}>{item.statut}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={() => setSelectedUser(null)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '26px 30px', width: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button onClick={() => setSelectedUser(null)} style={{ border: 'none', background: 'none', fontSize: 18, color: '#94A3B8', cursor: 'pointer' }}>✕</button>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#64748B', display: 'block', marginBottom: 6 }}>Entreprise associée</label>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={() => setInviteModal(false)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '26px 30px', width: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              <button onClick={() => setInviteModal(false)} style={{ border: 'none', background: 'none', fontSize: 18, color: '#94A3B8', cursor: 'pointer' }}>✕</button>
            </div>
            {[['Email professionnel', 'email', 'julie@entreprise.fr'], ['Nom complet', 'text', 'Julie Bernard']].map(([l, t, ph]) => (
              <div key={l} style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#64748B', display: 'block', marginBottom: 6 }}>{l}</label>
                <input type={t} placeholder={ph} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #E2E8F0', fontSize: 13, boxSizing: 'border-box', outline: 'none' }} />
              </div>
            ))}
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#64748B', display: 'block', marginBottom: 6 }}>Rôle</label>
