...
const TABS = ['Utilisateurs', 'Securite']
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, padding: '14px 22px', flexShrink: 0 }}>
        {[
          ...
          { label: 'Alertes', value: String(nbAlertes), bg: nbAlertes > 0 ? '#FEE2E2' : '#F1F5F9', tc: nbAlertes > 0 ? '#991B1B' : '#475569' },
          { label: 'RGPD', value: '100%', bg: '#F0FDF4', tc: '#065F46' },
          { label: 'Securite', value: secScore + '%', bg: '#DBEAFE', tc: '#1E40AF' },
        ].map((k, i) => (
      ...
      <div style={{ display: 'flex', borderBottom: '2px solid #EEF0F3', padding: '0 22px', background: '#fff', flexShrink: 0 }}>
        {TABS.map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={{ padding: '10px 16px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 13, fontWeight: activeTab === t ? 700 : 500, color: activeTab === t ? '#0A3D26' : '#94A3B8', borderBottom: activeTab === t ? '2px solid #0A3D26' : '2px solid transparent', marginBottom: -2 }}>{t}</button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px 22px' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {['Email', 'Role', 'Entreprise', 'Statut', 'Actions'].map(h => (
        ...
        {activeTab === 'Securite' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #EEF0F3', padding: '20px 22px' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0A3D26', marginBottom: 14 }}>Journal audit</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={() => setSelectedUser(null)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '26px 30px', width: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: '#0A3D26' }}>Droits</span>
