...
const ROLES = [
  { value: 'marque', label: 'Marque', desc: 'Donneur ordre, commande du fil' },
  { value: 'filature', label: 'Filature', desc: 'Transforme le coton en fil' },
  { value: 'fournisseur', label: 'Fournisseur coton', desc: 'Fournit la matiere premiere' },
]
    ...
    <div style={{
      ...
      <div style={{ width: 440 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 13, color: '#94A3B8', marginTop: 4 }}>Creer votre compte ETHYS</div>
        </div>
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #EEF0F3', padding: '32px', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
          <form onSubmit={handleRegister}>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#64748B', display: 'block', marginBottom: 10 }}>Votre role sur la plateforme</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {ROLES.map(r => (
                  <div key={r.value} onClick={() => setRole(r.value)} style={{ padding: '12px 14px', borderRadius: 10, cursor: 'pointer', border: `2px solid ${role === r.value ? '#0A3D26' : '#EEF0F3'}`, background: role === r.value ? '#F0FDF4' : '#FAFAFA', transition: 'all 0.15s' }}>
