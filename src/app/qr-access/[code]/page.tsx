...
export default async function QRAccessPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const t = await getTranslations('qrAccess')
    ...
    <div style={{ minHeight: '100vh', background: '#f5f3ef', fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ background: '#1a1a1a', padding: '20px 24px', color: '#fff' }}>
        <div style={{ maxWidth: 540, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 9, color: '#c2956e', fontWeight: 600, letterSpacing: 1, marginBottom: 4 }}>{t('badge')}</div>
            <div style={{ fontSize: 20, fontWeight: 800 }}>{cert?.reference ?? '—'}</div>
            <div style={{ fontSize: 11, opacity: 0.6, marginTop: 2 }}>{t('soustitre')}</div>
          </div>
      ...
      <div style={{ maxWidth: 540, margin: '0 auto', padding: '24px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #e8e3d8', padding: '20px', marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#8b7355', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14 }}>{t('sectionCert')}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              [t('lblFilature'), filature?.nom ?? '—'],
              [t('lblMarque'), marque?.nom ?? '—'],
              [t('lblVolumeTotal'), String(transfert.volume_kg) + ' kg'],
              [t('lblPctRecycle'), String(cert?.pct_recycle ?? 51) + '%'],
              [t('lblVolumeRecycle'), String(cert?.volume_recycle_kg ?? 0) + ' kg'],
              [t('lblVolumeVierge'), String(cert?.volume_vierge_kg ?? 0) + ' kg'],
              [t('lblEmiseLe'), cert?.date_emission ? new Date(cert.date_emission).toLocaleDateString('fr-FR') : '—'],
              [t('lblValideJusqu'), cert?.date_expiration ? new Date(cert.date_expiration).toLocaleDateString('fr-FR') : '—'],
            ].map(([label, val]) => (
        ...
        {qrCode ? (
          <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #c8d8b8', padding: '20px', marginBottom: 16, textAlign: 'center' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#2d5016', marginBottom: 16 }}>{t('titreQR')}</div>
            <img
            ...
            {dejaRecupere && (
              <div style={{ marginTop: 12, fontSize: 11, color: '#8b7355' }}>
                {t('recupereLe', { date: new Date(transfert.date_recuperation!).toLocaleDateString('fr-FR') })}
              </div>
            )}
          </div>
        ) : (
        ...
        <div style={{ fontSize: 10, color: '#a0aec0', textAlign: 'center', lineHeight: 1.6 }}>
          {t('mentionReserve')} <strong>{marque?.nom ?? t('votremarque')}</strong>.<br />
          {t('mentionInterdite')}<br />
          {t('mentionCertifie')} <a href="https://www.ethys-textileloop.com" style={{ color: '#8b7355' }}>ethys-textileloop.com</a>
        </div>
      </div>
    </div>
  )
